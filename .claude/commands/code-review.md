...
## Project Context (pass to both subagents)

- **App:** Persona Trainer — React 19 + TypeScript, Vite 6, Material UI v7
- **Styling:** MUI `sx` prop and `theme.ts` overrides — no Tailwind or CSS Modules
- **Backend:** Supabase (PostgreSQL + Auth + RLS), OpenAI API (gpt-4o, tts-1, whisper-1)
- **Testing:** Playwright E2E only (`npm run test:e2e` from `persona-trainer/`)
- **Lint:** `npm run lint` from `persona-trainer/`
- **Conventions:** Semicolons used; all DB types from `src/services/supabase/client.ts`; OpenAI calls via `openAIService` singleton; visibility queries must include `.or('is_public.eq.true,created_by.eq.USER_ID')`
